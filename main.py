...
from . import crud, models, database, auth, email_utils, rate_limit, cors, cloudinary_utils
...
@app.post("/register", response_model=models.User, status_code=status.HTTP_201_CREATED)
...
@app.get("/users/me", response_model=models.User, dependencies=[Depends(auth.get_current_active_user), Depends(rate_limit.limit_user_me)])
...
@app.post("/send-verification-email", status_code=status.HTTP_202_ACCEPTED, dependencies=[Depends(auth.get_current_active_user)])
async def send_verification(current_user: models.User, request: Request, db: Session = Depends(get_db)):
